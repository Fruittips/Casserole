package hintedHandoffLockStore
...
type hintedHandoffLockStore struct {
	Mu      sync.Mutex
	LockMap map[int]Handoff
}

var hhmutex hintedHandoffLockStore
...
func initHintedHandoff(nodeid int) {
	...
	initJson := []byte(`{"TableName": "hinted handoffs",
	"Columns": [
		"course_name",
		"num_of_slots"
	],
	...
	hhmutex = hintedHandoffLockStore{Mu: sync.Mutex{}, LockMap: make(map[int]Handoff)}
}

func (j *hintedHandoffLockStore) readfromHintedHandoff(nodeid int) Handoff {
...
func (j *hintedHandoffLockStore) getHandoffForRevivedNode(nodeid int, revivedId int) AtomicDbMessage {
	// get hint from hintedHandoff from this node
	allHints := j.readfromHintedHandoff(nodeid) //type handoff
...
func (j *hintedHandoffLockStore) writeToHintedHandoff(nodeid int, deadNodeId int, data AtomicDbMessage) bool {
	j.Mu.Lock()
	defer j.Mu.Unlock()
	...
	if j.LockMap == nil {
		j.LockMap = make(map[int]Handoff)
	}
	j.LockMap[nodeid] = allHints
...
func main() {
	...
	nodeHandoff := hhmutex.readfromHintedHandoff(1)
	...
	_ = hhmutex.writeToHintedHandoff(2, 0532, sampleData)

	// MUTEX LOCK INIT //

	// lock conditions:
	/* If cood probes node but it is not alive 		-- eq1
	 * If node revives								-- eq2
	 * Node asks to read hinted handoff first 		-- eq2 (node 2 has missing data)
	 * cood writes to hinted handoff				-- eq1
	 */
