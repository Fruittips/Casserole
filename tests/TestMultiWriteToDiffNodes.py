...
def TestMultiWriteToDiffNodes(runner: Runner) -> bool:
    """
    ...
    read_node_id = choice(list(runner.config.ring.keys()))
    ...
    for i in range(read_attempts):
        print(f"{step}. Read from node {read_node_id}")
        resp, success = read_req(read_node_id, course_id, student_id)
...
if __name__ == "__main__":
    ...
    try:
        ...
        start = time()
        success = TestMultiWriteToDiffNodes(runner)
        if success:
